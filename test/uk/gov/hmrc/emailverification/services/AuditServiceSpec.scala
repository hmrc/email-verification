...
class AuditServiceSpec extends UnitSpec with GuiceOneAppPerSuite with SessionCookieEncryptionSupport {
  ...
  def confirmDataEventSourceAndTags(dataEvent: DataEvent, expectedTransactionName: String): Assertion = {
  ...
  trait TestData {
    ...
    val passcodeExpireAt: Instant = Instant.now
    ...
    val passcodeDoc: PasscodeDoc = PasscodeDoc(
      sessionId = sessionId,
      email = emailAddress,
      passcode = passcode,
